...
public class OSPFAllLinkFailureEvaluation extends AbstractEvaluationFunction<ILinearRepresentation<Integer>>{
	...
	public OSPFAllLinkFailureEvaluation(NetworkTopology topology, Demands demands,LoadBalancer loadBalancer) {
		this(topology,demands,loadBalancer,LFObjectives.CONGESTION_CONGESTION);
	}
	
	
	public OSPFAllLinkFailureEvaluation(NetworkTopology topology, Demands demands,LoadBalancer loadBalancer, LFObjectives objectives) {
		super(false);
	...
	public OSPFAllLinkFailureEvaluation(NetworkTopology topology, Demands demands, LoadBalancer loadBalancer, double alfa) {
		 this(topology, demands, loadBalancer);
		 this.alpha=alfa;
	}
	...
	public double evaluate(ILinearRepresentation<Integer> solution) throws Exception {
		Double[] res=evaluateMO(solution);
		return (this.alpha*res[0]+(1-this.alpha)*res[1]);
	}
	...
	private Double[] evaluateLinkFailure(int[] weights) throws DimensionErrorException{
		...
		double r0=simul.evalWeights(weights,1,demands,null);
		...
		for(int i=0;i<t.getNetGraph().getNEdges();i++){
			...
			s.evalWeights(weights,1,demands,null);		
	...
	public Double[] evaluateLinkFailureMultiThread(int threads,int[] weights) throws InterruptedException, DimensionErrorException{
		...
		for(int i=0;i<threads;i++){
			...
			runnables[i]=new ComputeUnit(topology.copy(),Arrays.copyOf(weights,weights.length),demands,a);
			from = to+1;
			t--;
		}
	...
	private class ComputeUnit implements Runnable {
		...
		public ComputeUnit(NetworkTopology topology, int[] weights, Demands demands, int[] edges){
			this.topo=topology;
			this.w=weights;
			this.d= demands;
			this.e =edges;
		...
		public void run() {
			...
			for(int i=0;i<e.length;i++){
				...
				double c=0.0;
				try {
					c=simul.evalWeights(w,1,demands,null);
