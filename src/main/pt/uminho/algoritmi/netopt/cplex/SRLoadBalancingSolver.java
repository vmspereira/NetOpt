...
public class SRLoadBalancingSolver {

	/**
	 * Given a SR paths configuration optimizes the MLU by correcting traffic
	 * load balancing between parallel SR paths.
	 */
	...
	public SRLoadBalancingSolver(NetworkTopology topo, OSPFWeights weights, SRConfiguration srConf, Demands demands) {
		this.topology = topo;
	...
	public SRLoadBalancingSolver(NetworkTopology topo, OSPFWeights weights, Demands d) throws DimensionErrorException {
	...
	private double optimize(NetworkTopology topology, Demands demands, OSPFWeights weights,
			SRConfiguration srConf) throws IloException, DimensionErrorException {
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++) {
				if (i != j) {
					...
					if (p.size() > 0) {
						// there was at least one configured SR path from i to j
						//System.out.println();
	...
	public static void main(String[] args) {
		...
		try {
			...
			SRLoadBalancingSolver solver = new SRLoadBalancingSolver(topology,w,srConf,demands);
			...
			System.out.println("=======================\n"
					...
					+"\nCongestion after:"+congestion_after+" ("+a_cong_cnf
